...
from services.sender import send_push
from src.utils.circuit_breaker import CircuitBreaker
from src.utils.retry_backoff import backoff_delay

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("push_consumer")
...
async def handle_message(msg: AbstractIncomingMessage):
    async with msg.process(requeue=False):
        ...
        try:
            await circuit.call(send_push, payload)
            ...
            if retries < settings.max_retries:
                ...
                await msg.channel.default_exchange.publish(
                    ...
                    routing_key=settings.push_queue,
                )
            else:
                logger.error("Max retries reached. Moving %s to DLQ", request_id)
                await msg.channel.default_exchange.publish(
...
async def main():
    ...
    queue = await channel.declare_queue(settings.push_queue, durable=True)
    await queue.bind(exchange, routing_key="push")
    await queue.consume(handle_message, no_ack=False)
    logger.info("Push consumer started, listening on %s", settings.push_queue)
